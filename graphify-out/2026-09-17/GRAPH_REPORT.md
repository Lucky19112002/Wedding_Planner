...
## Corpus Check
- Corpus is ~34,690 words - fits in a single context window. You may not need a graph.
...
2. `DATABASE` - 19 edges
3. `PRD` - 18 edges
4. `DECISIONS` - 16 edges
5. `log` - 16 edges
6. `CHANGELOG` - 15 edges
7. `README` - 14 edges
8. `API` - 13 edges
9. `MIGRATIONS` - 13 edges
...
- `MIGRATIONS` --references--> `README`  [EXTRACTED]
  docs/MIGRATIONS.md → schema/README.md
...
## Knowledge Gaps
- **46 isolated node(s):** `Supabase PostgREST API`, `Invitation RPCs`, `Archive RPCs`, `Documentation version history`, `Phase 3.2 migration readiness` (+41 more)
...
- **What connects `Supabase PostgREST API`, `Invitation RPCs`, `Archive RPCs` to the rest of the system?**
